...
This is a branching discriminator map, not a delivery sequence:

1. A concrete observed/evidential gap supplies an **entry condition**.
2. The smallest authorized specimen tests one bounded question with one runner and explicit non-claims.
3. A human receives a post-experiment review: measured support, non-claims, pressure, and promotion decision.
4. **Retain if** and **Revise if** conditions decide whether a future candidate becomes eligible, is changed, or is deferred.

No helper is promoted merely because two specimens look similar. Promotion requires independent recurrence or demonstrated insufficiency with a named boundary to preserve.

### Candidate review checklist

Before any candidate is offered for human authorization, its brief must state:

- an entry condition and one bounded question;
- its smallest realization and explicit non-claims;
...
**Bounded question:** Can two isolated work contexts begin from one retained base material `R0`, retain sibling candidates `R1a`/`R1b` without overwrite, and let a fresh process reconstruct their declared parent/context relationship plus an explicit selected, rejected, or unselected state?
...
**Retain if:** Equivalent observable reconstruction survives the changed private realization; retain the boundary and continue to defer a named storage technology or shared substrate.

**Revise if:** The contract cannot remain intelligible across the change; record the missing relationship rather than selecting Git, a database, CAS, or another substrate in advance.

**Eligibility is not authorization:** This branch needs both demonstrated pressure and a separately authorized brief.
...
**Retain if:** A local record/control separates availability, attention, interpretation, and consequence without a router, subscription system, event model, or always-awake process.

**Revise if:** The pressure is only host-notification behavior or cannot be reproduced in a bounded context; fix/defer that local concern rather than inventing a session/messaging runtime.

**Eligibility is not authorization:** This branch requires observed pressure and a separately authorized brief.

## Deferrals and anti-patterns
...
- [`AGENTS.md`](../../AGENTS.md) — workshop authorization and no-foundation rules.
- [`docs/notes/em-embodiment-map-v0.md`](../notes/em-embodiment-map-v0.md) — present responsibility pressures and current realizations.
- [`docs/notes/em-boundary-hypotheses-v0.md`](../notes/em-boundary-hypotheses-v0.md) — boundary-first test lenses.
- [`docs/notes/resource-session-working-views-lineage-v0.md`](../notes/resource-session-working-views-lineage-v0.md) — resource/revision/working-view discriminator context.
- [`docs/roadmaps/machine-ecology-coordination-roadmap.md`](machine-ecology-coordination-roadmap.md) — withdrawn historical candidate, not this roadmap's predecessor or authority.
