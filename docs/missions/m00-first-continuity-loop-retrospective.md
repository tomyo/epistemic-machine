...
| C Persistence | `experiments/m00-e01-first-continuity-loop/.em/mission-0/ACCEPTED-CONSEQUENCE.json` | `bd01c74` |
...
Only one organism-owned thing had to survive: `ACCEPTED-CONSEQUENCE.json` at `bd01c74` (machine's own `.git` inside `experiments/m00-e01-first-continuity-loop/.em/`).
...
- **Workshop ≠ machine.** Root has no `.em`. Many machines ⇒ many scoped experiment `.em/` directories (later `machines/<name>/.em/`). Move `bd01c74` from root → `experiments/` made the folder story honest (`df9a9b3`).
...
That sketch appeared to offer transport keys, signatures, relays, and event/view separation, but it did not establish machine identity, a canonical event model, or a need for those mechanisms. Mission M00's filesystem bus **did not fail** — `verify-rebirth → 0`. Building the sketch then would have been a top-down stack, so no part was promoted.
...
The former instruction to wait for CL to choose Mission 1 is not current direction. CL may provide non-authoritative evidence and advice; EM owns source-local engineering dispositions; the human selects a future direction and separately authorizes bounded work. No Mission M01 is selected by this retrospective.

```
workshop (b6b79eb)                         machine per experiment
docs/missions/m00-first-continuity-loop.md ✓ CLOSED        experiments/m00-e01-first-continuity-loop/.em @ bd01c74
docs/missions/m00-first-continuity-loop-retrospective.md   └── mission-0/ACCEPTED-CONSEQUENCE.json
experiments/m00-e01-first-continuity-loop/ (ephemeral)      bus (inbox ↔ outbox) — only shared surface
